...
def start_replica():
    ...
    if '-n' in sys.argv[1:] or '--domainname' in sys.argv[1:]:
        if '-r' in sys.argv[1:] or '--route53' in sys.argv[1:]:
            ...
            # Check if nameserver can connect to dns_port
            cmd = ["ssh", "-t", nodemanager.username+'@'+i,
                   "python -c \"exec(\\\"import socket\\nsocket.socket(socket.AF_INET,socket.SOCK_STREAM).bind(('localhost', "+str(dns_port)+"))\\\")\""]
            ...
            output = ssh.stderr.readlines()
            if result:
                print "The Nameserver cannot bind to socket "+str(dns_port)+". Try another instance."
                print result
                return

    print "Starting replica on %s" % str(i)
    venv_path = '/home/students/cs091747/openreplica/openr-env/bin/activate'
    venv_command = ' '.join(['source',venv_path,';'])
    concoord_args = ' '.join(sys.argv[1:])
    concoord_cmd = ' '.join(['concoord replica',concoord_args])
    cmd = ["ssh", nodemanager.username+'@'+i,'bash -c "' ,venv_command, concoord_cmd, '"']
    print ' '.join(cmd)
    p = nodemanager._issuecommand(cmd)
