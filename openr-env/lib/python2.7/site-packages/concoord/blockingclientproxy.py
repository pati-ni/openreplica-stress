...
from concoord.leader import LeaderUpdate
...
class ClientProxy(LeaderUpdate):
    def __init__(self, bootstrap, timeout=60, debug=False, token=None):
        ...
        self.bootstrap_str = self.get_leader()
        self.bootstraplist = self.discoverbootstrap(self.bootstrap_str)
        #self.bootstraplist = self.get_leader_tuple()
    ...
    def getbootstrapfromdomain(self, domainname):
        tmpbootstraplist = []
        try:
            answers = resolver.query('_concoord._tcp.'+domainname, 'SRV')
    ...
    def trynewbootstrap(self):
        while self.bootstrap_str == self.get_leader() or self.bootstrap_str == '':
            print 'wait for configuration'
            time.sleep(0.2)
        print 'new  leader',self.get_leader()
        self.bootstrap_str = self.get_leader()

        self.bootstraplist = self.discoverbootstrap(self.bootstrap_str)
        print self.bootstraplist
        # if self.domainname:
        ...
        while True:
            ...
            try:
                ...
                for s in inputready:
                    ...
                    if reply is None:
                        print 'Reconfiguration needed: No reply received'
                        needreconfig = True
                        ...
                        with self.lock:
                            if reply.replycode == CR_OK or reply.replycode == CR_EXCEPTION or reply.replycode == CR_UNBLOCK:
                                ...
                                print 'Reconfiguration needed: Received', cr_codes[reply.replycode]
                                needreconfig = True
                            else:
