...
class Replica(Node):
    def __init__(self):
        ...
        try:
            ...
            self.token = None
        zk = KazooClient(controller_connection)

        zk.start()
        s = participant_node + '/' + str(socket.gethostname()) + ':' + str(self.port)
        zk.ensure_path(s)
        zk.set(s, str(time.time()))
        zk.stop()
        # leadership state
        ...
        # Quorum state
        if self.durable:
            self.file = open('/tmp/concoordlog', 'a')
...
# LEADER STATE
    def become_leader(self):
        ...
        """
        print 'Registering to zookeeper leader status'
        if not self.isleader:
            ...
            print 'Im becoming leader', time.time()
            zk = KazooClient(controller_connection)
            zk.start()
            zk.ensure_path(leader_node)
            zk.set(leader_node, str(socket.gethostname())+':'+str(self.port))
            zk.stop()
